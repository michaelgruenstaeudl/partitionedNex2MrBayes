...
# FUNCTIONS USED BY MULTIPLE SOFTWARE TOOLS

write_partitioneval_config_file()
#   This function generates a config file as required by PartitionFinder.
#   INP:  $1: path to temp folder ($tmpFOLDER)
#         $2: name of config file ($reformCFG)
#         $3: type of modeltesting selected ($typMDLTST)
#         $4: name of phylip-formatted file ($phylipFil)
#         $5: name of file with charsets lines ($charSetsL)
#         $6: boolean variable if userscheme only or not ($usrschmBOOL)
#   OUP:  writes a phylip-formatted file to disk
{
    ...
    charsetLines=$(cat ./$1/$5)

    # USING CONFIG TEMPLATE
    if [ $3 == "partitionfinder" ]; then
        CMDline=$(grep -A1 --ignore-case '^%PARTITIONFINDER_CFG:' ./$1/$2 | tail -n1)
    elif [ $3 == "partitiontest" ]; then
        CMDline=$(grep -A1 --ignore-case '^%PARTITIONTEST_CFG:' ./$1/$2 | tail -n1)
    fi
    CMDline=${CMDline//\[NAME_OF_ALIGNMENT\]/$4} # Substitution with Bash's built-in parameter expansion
...
#           `GTR+I`.
{
    ...
    do
        ...
        model_invgm=$(echo "$LINE" | awk '{print $2}' | awk -F'+' '{print $1=""; gsub(" ","+"); print $0}')
...
#   OUP:  writes the bestModls file to disk
{
    ...
    if [ -f "$pt_results" ]; then
        ...
        while IFS= read -r LINE; do 
            ...
            echo "Subset$partitionCounter $partitionModel [$partitionCONTENT]" >> ./$1/$2
        done <<< "$modelinf_str" # Using a here-string
    else
        echo -ne " ERROR | $(get_current_time) | No result files of the partitionfinding / modeltesting (*.bestModel) found.\n"
        exit 1
    fi
}
...
#   OUP:  none; generates an output folder named 'analysis' in temp folder
{
    ...
    if [ -f "./$1/partition_test.cfg" ]; then
        ...
        coreCMD=${CMDline//\[NAME_OF_CFG_FILE\]/partition_test.cfg}
...
if [ \("$typMDLTST" = "partitionfinder" ] && [ "$usrschmBOOL" -eq 1 \) -o \("$typMDLTST" = "partitiontest" ] && [ "$usrschmBOOL" -eq 1 \) ]; then
...
if [ "$typMDLTST" = "partitionfinder" -o "$typMDLTST" = "partitiontest" ]; then
    ...
    write_partitioneval_config_file $tmpFOLDER $reformCFG $typMDLTST $phylipFil $charSetsL $usrschmBOOL
fi
...
if [ "$typMDLTST" = "jmodeltest" ]; then
    ...
    modeltesting_via_partitiontest $tmpFOLDER $BinMDLTST $reformCFG
fi
...
if [ "$typMDLTST" = "jmodeltest" ]; then
    ...
    extract_modelinfo_from_partitiontest $tmpFOLDER $bestModls
    convert_models_into_lset $tmpFOLDER $bestModls $lsetDefns $charSetsL
elif [ "$typMDLTST" = "partitionfinder" ]; then
