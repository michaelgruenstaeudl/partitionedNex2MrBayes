...
#         file with name $partFname in temp folder
{
    ...
    while IFS= read -r line; do 
        ...
        partitFn1=$(printf %04d $myCounter)
        ...
        awk 'NR==FNR{start=$1;lgth=$2-$1+1;next} {print $1, substr($2,start,lgth)}' FS='-' <(echo "$charsetRanges") FS=' ' <(echo "$pureMatrx") >> $1/$partitnFn # AMAZING LINE! The actual string splitting is done here.
...
#   OUP:  write to the outfile
{
    ...
    #new_charsetRanges=$(echo "$new_charsetLines" | awk -F"=" '{print $2}' | sed 's/\;//')

    myCounter=0
    if [ ! -z "$new_charsetLines" ] && [ ! -z "$best_scheme" ]; then
        while IFS= read -r line; do 
            myCounter=$((myCounter+1))
            newPartitFn1=$(printf %04d $myCounter)
            ...
            awk 'NR==FNR{for(i=1;i<=NF;i++) {split($i,x,"-"); start[i]=x[1]; end[i]=x[2]}; print ""; n=NF; next} {printf "%s", $1 FS; for(i=1;i<=n;i++) printf "%s", substr($2,start[i],end[i]-start[i]+1); print ""}' <(echo "$charsetRanges") <(echo "$pureMatrx") > $1/$newPartitnFn # AMAZING LINE! The actual string splitting is done here.
        done <<< "$new_charsetLines" # Using a here-string

        # CONTINUE HERE !
        paste $1/newPartition_* > $3
        echo "\n[ $best_scheme ]\n" >> $3
        echo -ne "\nbegin sets;\n$new_charsetLines\nend;\n" >> $3
