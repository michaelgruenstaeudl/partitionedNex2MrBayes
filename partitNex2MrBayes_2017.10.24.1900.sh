...
#   OUP:  none; generates *.bestModel* output files in temp folder
{
    ...
    # LEGACY: CMDline=$(echo $CMDline | sed -e "s/\[PATH_TO_JMODELTEST_JAR\]/\$2/")
    ...
    then
        ...
        # LEGACY: main_cmd=$(echo $CMDline | sed -e "s|\[PARTITION_NAME\]|$file|") # NOTE: The variable $file contains protected characters (here forwardslashes), so I have to use another delimiter with sed.
        CMDline=${CMDline//\[PARTITION_NAME\]/$file} # Substitution with Bash's built-in parameter expansion
        eval "$CMDline 1>${file}.bestModel 2>&1" # Executin of main_cmd
...
#   OUP:  writes a phylip-formatted file to disk
{
    ...
    charsetLines=$(cat $1/$4)
    # LEGACY: charsetLines=$(cat $1/$4 | awk '{printf "%s\\n", $0}')
    # USING CONFIG TEMPLATE
    ...
    # LEGACY: CMDline=$(echo "$CMDline" | sed -e "s|\[NAME_OF_ALIGNMENT\]|$3|") # NOTE: The variable $3 contains protected characters, so I have to use another delimiter with sed.
    # LEGACY: CMDline=$(echo "$CMDline" | sed -e "s|||")
...
if [ "$typMDLTST" = "jmodeltest" ]; then
    assemble_mrbayes_block_from_jmodeltest $tmpFOLDER $NamOTFILE $lsetDefns $charSetsL $reformCFG
fi
